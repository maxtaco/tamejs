...
them as callbacks generated by `defer()`.  Callbacks generated by
`defer` are annotated to contain the file, line, and function where
they were created. More important, callbacks generated with `defer()`
also hold a reference to the callback `cb` passed to the current
function.  This creates an implicit stack that can be walked when an
exception is thrown.
...
"callstack", and then to exit.  Here's what happens when this program
is run:
