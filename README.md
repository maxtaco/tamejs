...
them as callbacks generated by `defer()`.  Callbacks generated
by `defer` are annotated to contained the file, line, and function
where they were created, and more important, the callback `cb` passed
to the function in which they were generated.  This creates an implicit
stack that can be walked when an exception is thrown.  
...
handler in node to print out the standard callstack, and also
the tame implicit callstack on catch, then to exit.  Here's what
happens when this program is run:
...
numbering relative to the translated file. The second stack trace
is much better.  It tells the sequence of tamed calls the lead
to this exception.
...
a list of call site desriptions.  You can call this from your
